...
if (!function_exists('slugToJikanQuery')) {
    function slugToJikanQuery(string $animeId): string
    {
        $cleaned = preg_replace('/-[a-z0-9]{3,}$/i', '', urldecode($animeId));
...
if (!function_exists('normalizeJikanCharacterPayload')) {
    function normalizeJikanCharacterPayload(array $jikanCharacters): array
    {
        $normalized = [];
        foreach ($jikanCharacters as $entry) {
            $character = $entry['character'] ?? [];
            $voiceActors = [];

            foreach (($entry['voice_actors'] ?? []) as $actor) {
                $person = $actor['person'] ?? [];
                $voiceActors[] = [
                    'id' => $person['mal_id'] ?? '',
            ...
            $normalized[] = [
                'character' => [
                    ...
                    'poster' => $character['images']['jpg']['image_url'] ?? '',
                    'cast' => $entry['role'] ?? '',
                ],
...
if (!function_exists('fetchAnimeDataFromJikan')) {
    function fetchAnimeDataFromJikan(string $animeId): array
    {
        ...
        $recommendationsResponse = detailsHttpGetJson('https://api.jikan.moe/v4/anime/' . $malId . '/recommendations');
        $charactersResponse = detailsHttpGetJson('https://api.jikan.moe/v4/anime/' . $malId . '/characters');

        $relations = ($relationsResponse['status'] === 200 && is_array($relationsResponse['json']['data'] ?? null))
            ? $relationsResponse['json']['data']
            : [];
        $recommendations = ($recommendationsResponse['status'] === 200 && is_array($recommendationsResponse['json']['data'] ?? null))
            ? $recommendationsResponse['json']['data']
            : [];
        $characters = ($charactersResponse['status'] === 200 && is_array($charactersResponse['json']['data'] ?? null))
            ? $charactersResponse['json']['data']
            : [];

        $genres = array_values(array_filter(array_map(
            static fn($genre) => $genre['name'] ?? null,
            $anime['genres'] ?? []
        )));

        $producer = array_values(array_filter(array_map(
            static fn($item) => $item['name'] ?? null,
            $anime['producers'] ?? []
        )));

        $studio = implode(', ', array_values(array_filter(array_map(
            static fn($item) => $item['name'] ?? null,
            $anime['studios'] ?? []
        ))));
        ...
        foreach ($recommendations as $entry) {
            ...
            $recommendedAnimes[] = [
                'id' => (string) ($recommendation['mal_id'] ?? ''),
                'name' => $recommendation['title'] ?? 'Unknown',
                'jname' => $recommendation['title'] ?? 'Unknown',
                'poster' => $recommendation['images']['jpg']['large_image_url']
                    ?? $recommendation['images']['jpg']['image_url']
                    ?? '',
                'type' => 'TV',
                'duration' => '',
                'episodes' => ['sub' => '', 'dub' => ''],
        ...
        if (!empty($anime['trailer']['embed_url'])) {
            $trailers[] = [
                ...
                'source' => $anime['trailer']['embed_url'],
        ...
        return [
            ...
            'anilistId' => null,
            'title' => $displayTitle,
            'japanese' => $anime['title_japanese'] ?? ($anime['title'] ?? ''),
            'synonyms' => implode(', ', $anime['title_synonyms'] ?? []),
            ...
            'premiered' => !empty($anime['season']) && !empty($anime['year'])
                ...
                : 'Unknown',
            'malscore' => (string) ($anime['score'] ?? 'N/A'),
...
$animeData = fetchAnimeDataFromJikan((string) $animeId);
if (empty($animeData)) {
    echo "Anime data not found.";
    exit;
}
$displayTitle = !empty($animeData['title']) ? $animeData['title'] : ($animeData['japanese'] ?? 'Unknown Title');
$japaneseTitle = $animeData['japanese'] ?? '';
$displayPoster = !empty($animeData['banner'])
    ? $animeData['banner']
    : (!empty($animeData['poster'])
        ? $animeData['poster']
        : ($animeData['image'] ?? ''));
$subCount = isset($animeData['subEp']) && $animeData['subEp'] !== '' && strtoupper((string) $animeData['subEp']) !== 'N/A'
    ? $animeData['subEp']
    : '?';
$dubCount = '?';
...
<title>Details Of <?= htmlspecialchars($displayTitle) ?> - <?= htmlspecialchars($websiteTitle) ?></title>

<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<meta name="title" content="Watch <?= htmlspecialchars($displayTitle) ?> - <?= htmlspecialchars($websiteTitle) ?>" />
...
<meta property="og:title" content="Details Of <?= htmlspecialchars($displayTitle) ?> - <?= htmlspecialchars($websiteTitle) ?>">
...
<meta itemprop="image" content="<?= htmlspecialchars($displayPoster) ?>">
<meta property="og:image" content="<?= htmlspecialchars($displayPoster) ?>">
<meta property="og:image:secure_url" content="<?= htmlspecialchars($displayPoster) ?>">
<meta property="og:image:width" content="650">
<meta property="og:image:height" content="350">
<meta property="twitter:title" content="Details Of <?= htmlspecialchars($displayTitle) ?> - <?= htmlspecialchars($websiteTitle) ?>">
                ...
                <div class="ani_detail-stage">
                    <div class="container">
                        <div class="anis-cover-wrap">
                            <div class="anis-cover" style="background-image: url('<?= htmlspecialchars($displayPoster) ?>')"></div>
                        </div>
                        <div class="anis-content">
                            <div class="anisc-poster">
                                <div class="film-poster">
                                    <img src="<?php echo htmlspecialchars($displayPoster); ?>" class="film-poster-img">
                            ...
                            <div class="anisc-detail">
                                <div class="prebreadcrumb">
                                    <nav aria-label="breadcrumb">
                                        <ol class="breadcrumb">
                                            ...
                                            <li class="breadcrumb-item dynamic-name active" data-jname="<?= htmlspecialchars($japaneseTitle) ?>"><?= htmlspecialchars($displayTitle) ?></li>
                                        </ol>
                                    </nav>
                                </div>
                                            ...
                                            <div class="dropdown-menu dropdown-menu-model dropdown-menu-normal">
                                                <?php foreach ($watchlistLabels as $statusId => $label): ?>
                                                    <a class="wl-item dropdown-item <?= ($watchlistStatus == $statusId) ? 'active' : '' ?>" 
                                                       ...
                                                       data-animename="<?= htmlspecialchars($displayTitle) ?>" 
                                                       data-poster="<?= htmlspecialchars($displayPoster) ?>" 
                                                       data-subcount="<?= htmlspecialchars($subCount) ?>" 
                                                       data-dubcount="<?= htmlspecialchars($dubCount) ?>" 
                                ...
                                <div class="film-text m-hide">
                                    <?= htmlspecialchars($websiteTitle) ?> is the best site to watch <strong><?= htmlspecialchars($displayTitle) ?></strong> SUB online, or you can even watch <strong><?= htmlspecialchars($displayTitle) ?></strong> DUB in HD quality.
                    ...
                    <div class="block_area-header">
                        ...
                        <div class="float-right viewmore">
                            <?php if (!empty($hiddenActors)): ?>
                                <a class="btn" href="javascript:;" id="toggle-characters">View more<i class="fas fa-angle-right ml-2"></i></a>
                            <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const toggleCharactersBtn = document.getElementById('toggle-characters');
    const hiddenCharacters = document.getElementById('characters-hidden-list');
    if (toggleCharactersBtn && hiddenCharacters) {
        toggleCharactersBtn.addEventListener('click', function (e) {
            e.preventDefault();
            ...
            toggleCharactersBtn.innerHTML = isHidden
                ? 'View less<i class="fas fa-angle-right ml-2"></i>'
...
<script>
    const characterData = <?= json_encode($characterData, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?>;
